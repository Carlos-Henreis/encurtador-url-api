...
public class UrlController {
    ...
    @PostMapping
    public ResponseEntity<UrlEncurtadaResponse> create(@RequestBody @Valid UrlEncurtadaRequest request) {
        UrlEncurtadaResponse response = urlUseCase.createShortUrl(request.getUrlOrigem());
    ...
    public ResponseEntity<EstatisticasResponse> getStatistics(@PathVariable String shortCode) {
        return ResponseEntity.ok(urlUseCase.getStatistics(shortCode));
    }

    @GetMapping("/qrcode/{shortCode}")
    public ResponseEntity<byte[]> getQrCode(@PathVariable String shortCode) throws Exception {
        byte[] qrCode = urlUseCase.getQrCodeLink(shortCode);
